...
class stBERT(nn.Module):
    ...
    def train(self, data,  ss_labels=None, position=None, data_save_path=None,
        ...
        with tqdm(range(self.epochs), desc='Training') as pbar:
            for epoch in pbar:
                ...
                with autocast():
                    masked_x, mask = mask_features(data.x, self.bert_mask_rate, rand_e=epoch)
                ...
                try:
                    ...
                    print(f"[Epoch {epoch}] Using device: {next(self.parameters()).device}")
                ...
                if eval and epoch > int(self.epochs - 30):
    ...
    def eval_bert(self, data, labels=None, reso=0.5,
        ...
        if method == "mclust":
            pred_mclust = mclust_R(embedding=pca_input, num_cluster=self.n_clusters)
        elif method == "louvain":
        ...
        if labels is not None:
            label_df = pd.DataFrame({"True": labels, "Pred": pred_mclust}).dropna()
